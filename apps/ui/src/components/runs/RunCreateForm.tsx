...
export function RunCreateForm() {
  ...
  const removeCompetitorUrl = (index: number) => setCompetitorUrls(competitorUrls.filter((_, i) => i !== index));
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const configurableSteps = stepConfigs.filter((s) => s.isConfigurable && s.stepId !== 'approval');
    const stepConfigsPayload = configurableSteps.map((step) => ({
      step_id: step.stepId,
  ...
  const inputClass = "w-full px-3 py-2.5 text-sm border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all";
  const labelClass = "block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5";

  const TOOLS = [
    { name: 'serp_fetch', label: 'SERP取得', desc: 'Google検索結果を取得', icon: Search, recommended: true },
    { name: 'page_fetch', label: 'ページ取得', desc: '競合ページの本文を取得', icon: Globe, recommended: true },
    { name: 'url_verify', label: 'URL検証', desc: '参照URLの有効性確認', icon: Shield, recommended: true },
    { name: 'pdf_extract', label: 'PDF抽出', desc: 'PDFからテキストを抽出', icon: FileText, recommended: false },
  ];
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="p-5 space-y-5">
          <div>
            ...
            <input
              type="text"
              {...register('keyword')}
              className={cn(inputClass, errors.keyword && 'border-red-300 dark:border-red-600 focus:border-red-500')}
              placeholder="SEO対策 初心者"
            />
            {errors.keyword ? (
              <p className="mt-1.5 text-sm text-red-600 dark:text-red-400">{errors.keyword.message}</p>
            ) : (
              <p className="mt-1.5 text-xs text-gray-500 dark:text-gray-400">上位表示を狙うキーワードを入力</p>
            )}
          </div>
                ...
                <>
                  {counts.gemini > 0 && (
                    <div className="flex items-center gap-2.5 px-4 py-2.5 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100">Gemini</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">{counts.gemini}工程</p>
                      </div>
                    </div>
                  )}
                  {counts.anthropic > 0 && (
                    <div className="flex items-center gap-2.5 px-4 py-2.5 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100">Claude</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">{counts.anthropic}工程</p>
                      </div>
                    </div>
                  )}
                  {counts.openai > 0 && (
                    <div className="flex items-center gap-2.5 px-4 py-2.5 bg-green-50 dark:bg-green-900/20 rounded-lg">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100">OpenAI</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">{counts.openai}工程</p>
                      </div>
                    </div>
                  )}
                </>
      ...
      <section className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">記事生成に使用する外部ツール</p>
        </div>
        <div className="p-5">
      ...
      <section className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">エラー時の挙動と品質管理</p>
        </div>
        <div className="p-5 space-y-5">
          <div>
          ...
          <div className="border-t border-gray-100 dark:border-gray-700 pt-5">
            <label className={cn(
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">自動修正を有効化</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 font-medium">
                    推奨
                  </span>
                </div>
