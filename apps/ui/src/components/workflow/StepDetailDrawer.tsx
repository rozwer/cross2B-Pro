...
import { X, CheckCircle, XCircle, Loader2, Clock, Pause, ChevronRight, FileText, AlertTriangle, RotateCcw, ExternalLink, Copy, Check } from 'lucide-react';
...
const SUB_STEPS: Record<string, Array<{ id: string; name: string; description: string }>> = {
  ...
  'step9': [
    { id: 'adjust-final', name: '最終調整', description: '検証結果に基づく修正' },
  ],
  'step10': [
...
export function StepDetailDrawer({ step, stepName: propStepName, onClose, onRetry, isOpen }: StepDetailDrawerProps) {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-3">
            <div className={cn(
              ...
              {status === 'completed' && <CheckCircle className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />}
              {status === 'running' && <Loader2 className="w-5 h-5 text-cyan-600 dark:text-cyan-400 animate-spin" />}
              {status === 'failed' && <XCircle className="w-5 h-5 text-red-600 dark:text-red-400" />}
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 font-mono">{effectiveStepName}</p>
            </div>
          </div>
          <button
            onClick={onClose}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        ...
                        {status === 'running' && (
                          <span className="text-xs text-cyan-600 dark:text-cyan-400">処理中...</span>
                        )}
                      </div>
                ...
                {attempts.map((attempt, index) => (
                  <div
                    ...
                    className={cn(
                      ...
                      attempt.status === 'succeeded' && "bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800",
                      attempt.status === 'failed' && "bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800",
                      attempt.status === 'running' && "bg-cyan-50 dark:bg-cyan-900/20 border-cyan-200 dark:border-cyan-800"
                    )}
                  >
              ...
              <div className="space-y-2 text-sm">
                ...
                {step.started_at && step.completed_at && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-700 dark:text-gray-300">
                      {((new Date(step.completed_at).getTime() - new Date(step.started_at).getTime()) / 1000).toFixed(1)}秒
                    </span>
                  </div>
                )}
              </div>
