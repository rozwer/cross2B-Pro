...
## フェーズ 4: UIでの表示改善 `cc:完了`

### 4.1 品質警告の詳細化

**ファイル**: `apps/ui/src/components/artifacts/StepContentViewer.tsx`

- [x] 「出力が切れている」警告に詳細情報を追加
- [x] 出力トークン数/期待トークン数を表示
- [x] 再実行を促すメッセージを追加

**実装内容**:
- `issueLabels`に`output_too_small`、`appears_truncated`を追加
- 切れ関連のissue検出ロジック（`hasTruncationIssue`）を追加
- 詳細情報パネル（出力サイズ、出力トークン数、期待値との比較）を追加
- 再実行推奨メッセージを表示

### 4.2 runリストでの警告表示

- [ ] 不完全な出力を持つrunにはアイコンを表示（将来対応）
- [ ] ツールチップで詳細を説明（将来対応）
...
- [x] `npm run lint` でエラーがないことを確認（既存警告のみ）
- [x] `npx tsc --noEmit` で型エラーがないことを確認
- [ ] 新規runで工程3Cが正常に完了することを確認（手動テスト推奨）
- [ ] 出力トークンが十分に多いことを確認（手動テスト推奨）

---

## 完了基準

- [x] finish_reasonの検証がログに出力される
- [x] 出力トークン比率が低い場合に警告が出る
- [x] step3cの出力最小サイズチェックが機能する
- [x] UIで品質警告がわかりやすく表示される
- [x] TypeScript エラーがない
- [x] lint エラーがない（既存警告のみ）
