...
- Activity 基底クラス: `apps/worker/activities/base.py`
- Activity 実装例: `apps/worker/activities/step1.py`〜`step10.py`
- Workflow 定義: `apps/worker/workflows/article_workflow.py`
- 出力スキーマ: `apps/worker/activities/schemas/`
...
class StepXActivity(BaseActivity):
    """工程X: 〇〇を実行"""

    @activity.defn
    async def run(self, input: StepXInput) -> StepXOutput:
        # 1. 冪等性チェック（既存出力があればスキップ）
        ...
        # 3. Storage に保存
        output_path = await self.save_to_storage(result)

        # 4. 参照のみ返す
        return StepXOutput(
            output_path=output_path,
            output_digest=self.compute_digest(result),
