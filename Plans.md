...
| 課題 | 詳細 |
|------|------|
| **情報が古い・実態と乖離** | ROADMAP.md が Step 1-8 形式だが実装は Phase 1-5 で完了 |
| **重複・散乱** | 工程仕様が workflow.md と step-improvements/ に分散 |
| **不要ファイルが多い** | 仕様書/_archive/ の旧仕様、古いレポート類 |
| **構成がわかりにくい** | docs/ と 仕様書/ の役割が不明確 |
...
フェーズ 1 (削除・アーカイブ)
    ↓
フェーズ 2 (ROADMAP.md)  ←→  フェーズ 3 (仕様書整合性)  [並列可能]
    ↓                              ↓
           フェーズ 4 (docs整理)
                    ↓
           フェーズ 5 (CLAUDE.md)
