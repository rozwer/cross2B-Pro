...
**プロンプト変更ではinit-db再実行は不要**（MinIO JSON保存）。

### 初回セットアップ（既存DBがある場合）

数週間以上前のDBを使っている場合、スキーマが古い可能性がある。以下のマイグレーションで最新化すること:

```bash
# 包括マイグレーション（冪等・何度実行しても安全）
cat scripts/migrations/004_catchup_all.sql | docker compose exec -T postgres psql -U seo -d seo_articles
```

これにより以下が適用される:
- `runs`テーブル: `step11_state`, GitHub連携カラム等の追加
- `api_settings`, `hearing_templates`, `help_contents`テーブル作成
- `steps`テーブル: UNIQUE制約・error_codeカラム
- LLMモデル一覧のシードデータ（gemini-2.5-pro等）

**完全に新規の場合**: `docker compose down -v && docker compose up -d` でinit-db.sqlが自動適用される（既存データは消える）。
